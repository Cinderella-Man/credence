defmodule Credence.Semantic.FixFnGuardPosition do
  ...
  The same diagnostic is emitted for shapes this rule does not rewrite; the
  `should_report?/2` phase hook keeps `analyze` honest by reporting an issue
  only when the fix would rewrite the source.

  - Misplaced `when` in `def`/`defp` heads (`def f(x when is_atom(x), y)`):
    only `fn` clauses are rewritten.
  - `fn` clauses with more than one misplaced `when`
  ...
  `fix/2` would actually rewrite the source. The diagnostic is also emitted
  for misplaced `when` in `def`/`defp` heads and for fn clauses with several
  `when`s, which this rule deliberately does not rewrite.
  """
  ...
  #
  # Clauses with more than one `when` element are skipped: the guards would
  # have to be merged, and `when g2 when g1` chaining means "or" where the
  # author almost certainly meant "and".
  defp find_misplaced_when(patterns) do
    when_indexes =
      for {{:when, _, _}, idx} <- Enum.with_index(patterns), do: idx

    case when_indexes do
