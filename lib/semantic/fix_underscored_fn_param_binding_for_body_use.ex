defmodule Credence.Semantic.FixUnderscoredFnParamBindingForBodyUse do
  @moduledoc """
  Fixes `undefined variable` errors caused by LLMs writing `_var` as a plain
  function parameter (intending "unused" style) but then referencing `var` (no
  underscore) in the body.
  ...
  where `_tv` is a parameter but `tv` (without underscore) is used in the body.
  The compiler emits `undefined variable "tv"` because `_tv` and `tv` are
  different bindings.

  The fix removes the underscore from the parameter name so the binding matches
  the body usage.

  This is distinct from `FixUnderscoredPatternBindingForBodyUse` (which handles
  case/pattern-match contexts) and `FixUndefinedUnderscoredBinding` (which
  handles destructured parameters like `{q_start, _q_finish}`).  This rule
  targets plain underscore-prefixed parameters in a def/defp clause head.
  """
  ...
  def to_issue(%{message: msg, position: position}) do
    %Issue{
      rule: :fix_underscored_fn_param_binding_for_body_use,
      message: msg,
  ...
  # function clause, covering both the parameter declaration and all body usages.
