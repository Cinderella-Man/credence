defmodule Credence.Syntax.FixPythonModulo do
  ...
  strings — `IO.puts("100% done")` became `IO.puts("rem(100, done)")`, which
  ...
  emit `a * rem(b, 2)` — which parses, compiles, and quietly computes a
  ...
  # guard did nothing — `assert %Issue{}` became `rem(assert, Issu)e{}`, worse
  ...
  defp fixable_matches(shadow) do
    ...
    |> Enum.reject(fn [_whole, {left_start, _}, _right] ->
      precedence_hazard?(shadow, left_start)
    end)
  end
  ...
  defp fix_line(line, shadow) do
    {chunks, pos} =
      shadow
      |> fixable_matches()
      |> Enum.reduce({[], 0}, fn [{ms, ml}, {ls, ll}, {rs, rl}], {acc, pos} ->
        before = binary_part(line, pos, ms - pos)
        left = binary_part(line, ls, ll)
        right = binary_part(line, rs, rl)
        {[acc, before, "rem(", left, ", ", right, ")"], ms + ml}
      end)
