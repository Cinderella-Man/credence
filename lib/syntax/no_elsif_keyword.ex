defmodule Credence.Syntax.NoElsifKeyword do
  @moduledoc """
  Detects and rewrites Python/Ruby-style `elsif` chains to idiomatic `cond`.

  LLMs translating from Python (`elif`) or Ruby (`elsif`) emit `elsif` which
  is not valid Elixir syntax. The `elsif` token causes the parser to
  misinterpret block boundaries, producing misleading "missing terminator: end"
  errors. Converting `if`/`elsif`/`else` chains to `cond` is
  ...
  def analyze(source) do
    ...
    |> Enum.flat_map(fn {line, line_no} ->
      if Regex.match?(@elsif_re, line) do
        [
          %Issue{
            rule: :no_elsif_keyword,
            message: "Use `cond` instead of `elsif`",
            meta: %{line: line_no}
          }
        ]
      else
  ...
  defp find_elsif(lines) do
    Enum.find_value(Enum.with_index(lines), :not_found, fn {line, idx} ->
      if Regex.match?(@elsif_re, line), do: {:ok, idx}, else: nil
    end)
  end
  ...
  defp rewrite_block(lines, elsif_idx) do
    ...
    with {:ok, if_idx} <- scan_back_for_if(lines, elsif_idx - 1, elsif_indent),
         {:ok, branches, end_idx, else_branches} <-
           collect_branches(lines, if_idx, elsif_idx, elsif_indent) do
  ...
  defp collect_branches(lines, if_idx, elsif_idx, elsif_indent) do
    ...
    if_body = Enum.slice(lines, (if_idx + 1)..(elsif_idx - 1))
  ...
  defp collect_elsif_branches(lines, idx, target_indent, acc) do
    ...
    if Regex.match?(@elsif_re, line) and get_indent(line) == target_indent do
      case extract_condition_from_elsif(line) do
        {:ok, cond} ->
          next_idx = find_next_else(lines, idx + 1, target_indent)
          body = Enum.slice(lines, (idx + 1)..(next_idx - 1))
          collect_elsif_branches(lines, next_idx, target_indent, acc ++ [{cond, body}])
  ...
  defp find_next_else(lines, idx, target_indent) do
    Enum.find_value(idx..(length(lines) - 1), length(lines) - 1, fn i ->
      ...
      if line != nil and get_indent(line) == target_indent and
           (Regex.match?(@elsif_re, line) or Regex.match?(@else_re, line) or
