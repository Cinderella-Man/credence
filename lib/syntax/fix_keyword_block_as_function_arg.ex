defmodule Credence.Syntax.FixKeywordBlockAsFunctionArg do
  ...
  # Use Sourceror to find the column of the `if`/`unless` keyword.
  ...
  # Starting from the `if`/`unless` keyword position (0-indexed), find the last
  # keyword label (`else:` or `do:`) and walk through its value. Returns the
  # 0-indexed position of the last character of the value.
  defp find_last_keyword_value_end(chars, if_pos_0) do
    len = length(chars)
    # Search for ALL keyword positions after the if/unless
