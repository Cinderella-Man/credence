defmodule Credence.Syntax.NoElseIf do
  ...
  defp rewrite_block(lines, else_if_idx) do
    ...
    with {:ok, if_idx} <- scan_back_for_if(lines, else_if_idx - 1, else_if_indent),
         {:ok, branches, end_idx, else_branches} <-
           collect_branches(lines, if_idx, else_if_idx, else_if_indent) do
      ...
      # `:not_found` (no matching `if`) or `:bail` (a branch condition could not
      # be extracted cleanly — e.g. a multi-line condition or a trailing comment
      # after `do`). Emitting a guessed `true` clause there would silently change
      # behaviour, so we leave the source untouched and let it stay flagged.
      _ -> lines
    end
  end
  ...
  defp collect_branches(lines, if_idx, else_if_idx, else_if_indent) do
    ...
    with {:ok, if_cond} <- extract_condition(if_line),
         {:ok, branches, rest_idx} <-
           collect_else_if_branches(lines, else_if_idx, else_if_indent, [{if_cond, if_body}]) do
      case find_else_at_indent(lines, rest_idx, else_if_indent) do
        ...
        {:not_found, end_idx} ->
          # No trailing `else`: a nested `if` without an `else` evaluates to `nil`
  ...
  defp collect_else_if_branches(lines, idx, target_indent, acc) do
    ...
    if Regex.match?(@else_if_re, line) and get_indent(line) == target_indent do
      case extract_else_if_condition(line) do
        {:ok, cond} ->
