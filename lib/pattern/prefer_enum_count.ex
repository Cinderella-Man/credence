defmodule Credence.Pattern.PreferEnumCount do
  ...
  defp counting_fn?({:fn, _, [{:->, _, [[elem_var, acc_var], body]}]}) do
    var?(elem_var) and var?(acc_var) and counting_if?(body, elem_var, acc_var)
  end

  defp counting_fn?(_), do: false

  defp counting_if?({:if, _, [condition, clauses]}, elem_var, acc_var) when is_list(clauses) do
    do_body = extract_keyword_clause(clauses, :do)
    else_body = extract_keyword_clause(clauses, :else)

    acc_increment?(do_body, acc_var) and same_var?(else_body, acc_var) and
      predicate_safe?(condition, elem_var, acc_var)
  end

  defp counting_if?(_, _, _), do: false

  # The condition becomes the body of an `Enum.count/2` capture, with the element
  # variable rewritten to `&1`. That is only equivalent when the condition:
  #   * references the element variable (otherwise `&(...)` has no `&1` and is a
  #     compile error: "invalid args for &"), and
  #   * does NOT reference the accumulator variable (otherwise the original
  #     depends on the running count — e.g. `if acc < 3` caps the result — and
  #     the rewritten `acc` is an unbound variable; not the same answer), and
  #   * contains no `&` capture (wrapping it in `&(...)` would make a nested
  #     capture: "nested captures are not allowed" — a compile error).
  defp predicate_safe?(condition, {elem_name, _, _}, {acc_name, _, _}) do
    var_used?(condition, elem_name) and not var_used?(condition, acc_name) and
      not has_capture?(condition)
  end

  defp has_capture?(ast) do
    {_ast, found?} =
    ...
    if var?(elem_var) and var?(acc_var) do
      case body do
        {:if, _, [condition, clauses]} when is_list(clauses) ->
          ...
          if acc_increment?(do_body, acc_var) and same_var?(else_body, acc_var) and
               predicate_safe?(condition, elem_var, acc_var) do
