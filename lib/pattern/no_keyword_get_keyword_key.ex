defmodule Credence.Pattern.NoKeywordGetKeywordKey do
  ...
  keyword list with exactly one element. Three-argument calls and piped calls
  are not flagged.
  ...
  # Detect: Keyword.get(keywords_call) with exactly 2 args where the second is
  # a keyword list with exactly one element.
  defp detect({{:., _, [{:__aliases__, _, [:Keyword]}, :get]}, meta, [_list, kw_list]}) do
    if keyword_literal?(kw_list), do: {:ok, meta}, else: :skip
  end
  ...
  defp detect_fix(
         {{:., _, [{:__aliases__, _, [:Keyword]}, :get]}, _meta, [list, kw_list]} = node
       ) do
    case extract_keyword_pair(kw_list) do
      {:ok, atom_key, default_value} ->
        new_node =
