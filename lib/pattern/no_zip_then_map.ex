defmodule Credence.Pattern.NoZipThenMap do
  ...
  Guarded fns are flagged but not auto-fixed.
  """
      ...
      Macro.prewalk(ast, [], fn
        ...
        acc ->
          with true <- two_tuple_fn?(fn_arg),
               ...
               true <- is_enum_zip?(zip_node) do
            guarded = guarded_fn?(fn_arg)
            {node, [build_issue(map_meta, guarded) | acc]}
          else
            _ -> {node, acc}
          end
  ...
  # Enum.zip(a, b) — 2-arg form (at start of pipe or standalone)
  defp is_enum_zip?({{:., _, [{:__aliases__, _, [:Enum]}, :zip]}, _, [_, _]}), do: true
  # Enum.zip(b) — 1-arg form (in pipe context, first arg comes from pipe)
  defp is_enum_zip?({{:., _, [{:__aliases__, _, [:Enum]}, :zip]}, _, [_]}), do: true
  defp is_enum_zip?(_), do: false
  ...
  defp build_issue(meta, guarded?) do
    suffix = if guarded?, do: " Guarded fn — auto-fix skipped.", else: ""
