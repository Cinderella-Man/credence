defmodule Credence.Pattern.NoFilterThenFirst do
  @moduledoc """
  Performance rule (fixable): Detects `Enum.filter/2` or `Stream.filter/2`
  piped into `Enum.at/2` with a literal `0` index.

  The combination filters the entire enumerable then takes the first match,
  when `Enum.find/2` does the same in a single pass and stops at the first
  match.

  ## Flagged

      Enum.filter(numbers, &(&1 > 10)) |> Enum.at(0)
      Stream.filter(numbers, &(&1 > 10)) |> Enum.at(0)
      Enum.at(Enum.filter(numbers, &(&1 > 10)), 0)
      Enum.at(Stream.filter(numbers, &(&1 > 10)), 0)

  ## Not flagged

      Enum.filter(numbers, &(&1 > 10)) |> Enum.at(1)   # not index 0
      Enum.filter(numbers, &(&1 > 10)) |> Enum.at(0, -1) # has default arg
  ...
  # Nested: Enum.at(Enum.filter(coll, pred), 0)
  defp build_patch(
  ...
  defp check_node(
         {{:., _, [{:__aliases__, _, [:Enum]}, :at]}, meta,
          [filter_call, {:__block__, _, [0]}]}
       ) do
    if filter_step?(filter_call) do
  ...
  defp filter_module?({:__aliases__, _, [:Enum]}), do: true
