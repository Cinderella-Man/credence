defmodule Credence.Pattern.PreferMapsetForSetEquality do
  ...
  # Safe in all three families: matches only a 3-statement block of two `var =
  # String.codepoints/graphemes(...) |> Enum.uniq() |> Enum.sort()` bindings
  # plus their `==`; those calls are not DSL-expression code. The source scan
  # flags this rule because the construct appears in it, but the matcher cannot
  # reach a DSL expression, so the deliberate answer is the empty list rather
  # than an allowlist entry — the fixture-level oracle does not flag it at all.
  @impl true
