defmodule Credence.Pattern.PreferLookupForDigitConversion do
  ...
  defp build_replacement(name) do
    ...
    # String.at/2 would instead return values for out-of-domain input
    # (16 -> nil, -1 -> "F"), changing the function's behaviour. The guard
    # `remainder in 0..15` restores the exact original domain (is_integer and
  ...
  defp extract_hex_mapping(clauses) do
    ...
    if Enum.any?(mappings, &(&1 == :error)) do
      ...
      # Reverse before Map.new so the FIRST clause wins on duplicate keys,
      # matching Elixir's first-match clause dispatch (a later duplicate clause
      # is unreachable at runtime). Without this, a leading `hex_digit(5), "X"`
      # shadowed by a trailing `hex_digit(5), "5"` would be read as "5" and the
      # mapping would falsely look complete.
      {:ok, mappings |> Enum.reverse() |> Map.new()}
    end
  end
