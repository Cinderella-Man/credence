defmodule Credence.Pattern.PreferPatternMatchOverConditionalInRecursiveCount do
  ...
  defp extract_fixable({:def, meta, [{name, _, params} | rest]})
    ...
    with {:ok, head_var, tail_var} <- unwrap_cons(first_param),
         ...
         {:ok, count_var, cond_var, _op} <- extract_count_if(body, head_var),
         true <- recursive_tail_call?(body, name, tail_var, count_var) do
      {:ok,
       %{
  ...
  # Check the last expression is `count + fn_name(tail, ...)`.
  defp recursive_tail_call?(body, name, tail_var, count_var) do
    last = last_expr(body)
    ...
    case last do
      {:+, _,
       [
         {count_name, _, nil},
         {fn_name, _, [{t_name, _, nil} | _rest_args]}
       ]}
      when count_name == count_var and fn_name == name and t_name == tail_var ->
        true
  ...
  defp build_patch(original_node, info) do
    %{name: name, head_var: head_var, tail_var: tail_var, cond_var: cond_var, params: params} =
      info
