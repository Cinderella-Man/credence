...
#
# A "set" is the FIRST file listed in maintainer_tools/unfixable_unreviewed.md (the rule file) plus any
...
#
# It does NOT modify maintainer_tools/unfixable_unreviewed.md — that list is your progress tracker, so
...
if [[ -z "$first" ]]; then
  echo "maintainer_tools/unfixable_unreviewed.md is empty — nothing left to do."
  exit 0
fi

# Derive the base rule name and its grouped test files (longest-rule-base-prefix
# -wins; see promote_lib.sh). Captures multi-variant tests while stopping a short
...
echo "copied ${copied} file(s)$([[ $missing -gt 0 ]] && echo ", ${missing} missing")  —  ${remaining} line(s) left in unfixable_unreviewed.md"
