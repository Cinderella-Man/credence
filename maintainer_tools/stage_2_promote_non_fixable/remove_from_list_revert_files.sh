...
#   3. removes their lines from maintainer_tools/unfixable_unreviewed.md.
#
# Everything else — staged shared-file edits, maintainer_tools/unfixable_unreviewed.md's own diff — is
...
# 3: drop the reverted files' lines from maintainer_tools/unfixable_unreviewed.md (exact whole-line match).
tmp="$(mktemp)"
printf '%s\n' "${files[@]}" > "$tmp"
...
echo "reverted ${reverted} file(s)  —  removed $((before - after)) line(s) from unfixable_unreviewed.md, ${after} left"
