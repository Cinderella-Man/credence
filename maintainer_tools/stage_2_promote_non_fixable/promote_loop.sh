#!/usr/bin/env bash
#
# promote_loop.sh — orchestrator for the stage-2 stub-promotion loop.
#
# Stage 2 is a near-identical copy of stage 1 (review_loop.sh) over a FLAT path-
# list queue (maintainer_tools/unfixable_unreviewed.md = the check-only stubs that
# stage 1 auto-classified unfixable and never reviewed). Drives one fresh,
# sandboxed (no-git) Claude session per stub; the wrapper owns ALL git and ALL
# list edits; the session's only output channel is maintainer_tools/_verdict
# (ACCEPT | UNFIXABLE: <reason> | FOLLOWUP: <reason>) — a THREE-way verdict.
#
# Per iteration: self-heal a stale tree → pick the top stub → copy it in →
# classify (always greenfield for a copied stub) + build a briefing → run the
# session → read the verdict →
#   ACCEPT     → re-verify gate (incl. "fix is now real") → keep files, commit
#                "<base>: promoted", push.
#   UNFIXABLE: → revert files, append unfixable_confirmed.md, commit
#                "<base>: confirmed unfixable", push.
#   FOLLOWUP:  → revert files, append followup.md, commit "<base>: followup", push.
#   gate-fail / unrecognized verdict → followup (NEVER confirmed).
#   no verdict (crash / token-limit) → transient: revert, stay on row, retry.
#
# Usage:   promote_loop.sh [cap] [wait_min]
#   cap        max iterations (0 = run until the queue is empty; default 0)
...
# shellcheck source=promote_lib.sh
source "$SCRIPT_DIR/promote_lib.sh"

CANDIDATES="$REPO/maintainer_tools/unfixable_unreviewed.md"
STAGE1_CANDIDATES="$REPO/maintainer_tools/candidates.md"
CONFIRMED="$REPO/maintainer_tools/unfixable_confirmed.md"
FOLLOWUP="$REPO/maintainer_tools/followup.md"
VERDICT="$REPO/maintainer_tools/_verdict"
PROMPT="$SCRIPT_DIR/promote_prompt.md"
...
for f in "$CANDIDATES" "$FOLLOWUP" "$CONFIRMED" "$PROMPT"; do
  [[ -f "$f" ]] || { echo "error: required file not found: $f" >&2; exit 1; }
done
[[ -d "$SISTER" ]] || { echo "error: sister dir not found: $SISTER" >&2; exit 1; }

# Startup guard: stage 2 runs only after stage 1 has fully drained its queue.
if grep -q -v '^[[:space:]]*$' "$STAGE1_CANDIDATES" 2>/dev/null; then
  echo "error: stage 1 not done — $STAGE1_CANDIDATES is non-empty." >&2
  echo "       Drain candidates.md with stage_1's review_loop.sh first." >&2
  exit 1
fi

log() { printf '[promote_loop] %s\n' "$*"; }
die() { printf '[promote_loop] FATAL: %s\n' "$*" >&2; exit 1; }
...
# they appear in the queue (whatever was/will be copied in).
set_paths() {
...
accept_commit() {
  ...
  rlog ACCEPT "stripped set lines from the queue (kept files)"
  ...
  git -C "$REPO" commit -q -m "${base}: promoted"
  G_FILES="$(diffstat_head)"; G_SUITE="$(suite_summary)"
  rlog ACCEPT "committed '${base}: promoted' (${G_FILES}, suite ${G_SUITE})"
  do_push "${base}: promoted"
  rlog PUSH "${G_PUSH}"
}

# Confirmed-unfixable handler — a clone of followup() that records to
# unfixable_confirmed.md instead. Reached ONLY on an explicit UNFIXABLE: verdict
# (the agent proved no safe core for any shape). Reverts the in-set files and
# strips the stub's lines from the queue, exactly like followup().
confirmed_unfixable() {
  ...
  rlog CONFIRMED "reason: ${reason}"
  "$SCRIPT_DIR/remove_from_list_revert_files.sh" >/dev/null 2>&1 || true
  rlog CONFIRMED "reverted in-set files + stripped ${#paths[@]} stub line(s) from the queue"
  ...
  git -C "$REPO" add -- "$CANDIDATES" "$CONFIRMED" >/dev/null
  git -C "$REPO" commit -q -m "${base}: confirmed unfixable"
  G_FILES="reverted ${#paths[@]}f"
  rlog CONFIRMED "committed '${base}: confirmed unfixable'"
  do_push "${base}: confirmed unfixable"
  rlog PUSH "${G_PUSH}"
}
...
followup() {
  ...
  # any split tests the agent created) and strip their lines from the queue.
  ...
  "$SCRIPT_DIR/remove_from_list_revert_files.sh" >/dev/null 2>&1 || true
  rlog FOLLOWUP "reverted in-set files + stripped ${#paths[@]} set line(s) from the queue"
...
route_orphan() {
  ...
  rlog FOLLOWUP "orphan test → followup.md, stripped from the queue, committed"
...
main() {
  ...
  while :; do
    self_heal
    if list_empty; then log "done — the queue empty"; break; fi
    ...
    case "$verdict" in
      ...
      UNFIXABLE:*) G_VERDICT="CONFIRMED"; G_REASON="${verdict#UNFIXABLE:}"
                   confirmed_unfixable "$base" "$anchor" "${verdict#UNFIXABLE:}" ;;
      ...
      *)
        ...
        # until Claude recovers. The candidate is left untouched in the queue.
        retry=$((retry + 1))
    ...
    rm -f "$VERDICT"
    iter=$((iter + 1))
    list_empty && { log "done — the queue empty"; break; }
