...
## no_or_in_case_pattern — 2026-07-27
- Files:
  - `lib/syntax/no_or_in_case_pattern.ex`
- Reason: exact duplicate of the live Credence.Semantic.NoOrInCasePattern (same rule id, byte-identical fix output) and in a phase that can never invoke it — `case v do nil or "" -> :empty end` PARSES, so Credence.Syntax.analyze/2 returns [] and fix_with_trace/2 skips the pipeline; it also shipped with zero tests (5 syntax_meta failures).

## no_pipe_into_arithmetic_operator — 2026-07-27
- Files:
...
- Reason: Wrong phase (dead in production) and its fix emits non-parsing code on chained arithmetic — `x |> foo() / bar()` PARSES (verified: `Code.string_to_quoted` returns `{:ok, ...}`; the failure is the macro-expansion CompileError "cannot pipe list into Enum.sum() / length(list), the :/ operator can only take two arguments"), but `Credence.Syntax.analyze/2` returns `[]` and `fix_with_trace/2` skips the whole pipeline whenever the source parses, while this rule's own analyze/fix bail out on `{:error, _}` — so it can only ever act on parseable source, i.e. exactly when the phase never invokes it; its home is the Semantic phase (a new `lib/semantic/no_pipe_into_arithmetic_operator.ex` with `match?/1`+`to_issue/1`+`fix/2`), a different set and out of scope here. The only way it fires today is by accident, mid-reduce, on a file whose *unrelated* parse error an earlier-priority rule repaired — and there it corrupts code: the guard `is_tuple(call) and is_list(elem(call, 2))` accepts an arithmetic node as the "function call", so left-associated chains splice the piped value into the operator itself — verified `x |> foo() / bar() / baz()` -> `/(x, foo(), bar()) / baz()`, `x |> foo() + bar() + baz()` -> `+(x, foo(), bar()) + baz()`, `x |> foo() * bar() + baz()` -> `*(x, foo(), bar()) + baz()`, all of which do NOT parse, and `list |> Enum.sum() / length(list) * 100` is a mainstream shape. A unary left operand is silently miscompiled rather than broken loudly: `x |> -foo() + bar()` -> `x - foo() + bar()`, which parses and compiles but makes the piped value the left operand of a subtraction — a different program. A safe core does exist (left operand is a genuine, non-operator function call, no nested pipe-into-arithmetic — that is what the shipped tests cover), but narrowing to it still leaves a rule its own phase can never invoke.

## no_rescue_option_in_case — 2026-07-27
- Files:
...
- Reason: Duplicate of the live Credence.Semantic.FixAfterOrRescueInCase, in a phase that can never invoke it, and its line-based surgery corrupts the assignment shape the duplicate handles correctly — `case x do ... rescue e -> e end` PARSES (verified: `Code.string_to_quoted` returns `{:ok, ...}` with `rescue:` sitting in the case's keyword options; the failure is the compile-time diagnostic `unexpected option :rescue in "case"`), so `Credence.Syntax.analyze/2` returns `[]` and `fix_with_trace/2` skips the whole pipeline whenever the source parses — verified `Credence.Syntax.analyze/2` on the rule's own headline test input returns `[]` — while the rule's own `find_case_rescue/1` bails to `:none` on `{:error, _}`, so it can only ever act on parseable source, i.e. exactly when the phase never invokes it; it fires in production only by accident, mid-reduce, on a file whose *unrelated* parse error an earlier-priority rule repaired. Its home is the Semantic phase, and that home is already occupied: `lib/semantic/fix_after_or_rescue_in_case.ex` matches this exact diagnostic string (`@match_rescue "unexpected option :rescue in \"case\""`) and applies the same transform (wrap the `case` in `try`, move `rescue` out), so accepting this adds nothing but a second, worse implementation. Worse because the fix is line/indent string surgery rather than AST surgery: on the mainstream shape `y = case x do 1 -> :one; _ -> :other rescue e -> e end` it emits `try do\n  y = case x do ... end\nrescue ... end\ny`, dragging the assignment *inside* the `try` — verified this does NOT compile (`error: undefined variable "y"`, since `try` does not leak bindings), i.e. it turns a compile error into a different compile error; the live semantic rule given the identical input correctly emits `y =\n  try do\n    case x do ... end\n  rescue ... end`. The tests never catch this because all five fix tests use the same statement-position `case` with no binding, and two of them assert only `analyze(fix(...)) == []` / `valid_syntax?(fix(...))` rather than the whole output string.

## no_rescue_or_catch_outside_try — 2026-07-27
- Files:
...
- Reason: Targets valid, idiomatic code in a phase that only ever sees malformed code, and rewrites it in exactly the direction Credo (this project's own dep) flags — `def f(x) do x rescue _ -> :error end` PARSES and COMPILES (verified: `Code.string_to_quoted` returns `{:ok, ...}`, and the compiled `verify(nil)` returns `:error`), because this is Elixir's documented implicit `try`, not a syntax error at all; so `Credence.Syntax.analyze/2` returns `[]` and `fix_with_trace/2` skips the whole pipeline whenever the source parses — verified on the rule's own headline test input that `Credence.Syntax.analyze(src, []) == []` and `Credence.Syntax.fix_with_trace(src, []) == {src, []}` — while the rule's own `find_def_with_bad_clause/1` bails to `:none` on `{:error, _}`, so it can only ever act on parseable source, i.e. exactly when the phase never invokes it; it fires in production only by accident, mid-reduce, on a file whose *unrelated* parse error an earlier-priority rule repaired. Worse, the premise is inverted: `deps/credo/lib/credo/check/readability/prefer_implicit_try.ex` (EX3017, already a dependency here) uses this rule's "Bad (non-idiomatic)" moduledoc example as its **Good** and this rule's "Good" as its **Bad**, shape-for-shape — so the fix converts preferred code into the form the project's own linter reports. The `:else` arm resolves nothing either: Elixir warns `"else" shouldn't be used as the only clause in "def", use "case" instead` on the input, and the fixed output draws the identical warning re-pointed at `"try"` (verified both compile). The transform itself is semantically sound where it does run (verified heredoc bodies survive re-indentation byte-identically and the multiline-def-head case still parses, only misindented), so the failure mode is not corruption but a no-op rule pointed backwards: its home is a readability/pattern check that removes an explicit whole-body `try`, which is a different rule in a different phase and out of scope here.

## prefer_div_function_over_infix — 2026-07-27
- Files:
...
- Reason: Duplicate of the live Credence.Syntax.FixDivRem, in a phase that can never invoke it on its own headline example, and its only production effect is to defeat FixDivRem's deliberate capture guard — the premise is false: `a div b` PARSES (verified: `Code.string_to_quoted("def divide(a, b), do: a div b")` returns `{:ok, ...}` with body `{:a, _, [{:div, _, [{:b, _, nil}]}]}`, i.e. Elixir's unparenthesized call `a(div(b))`, not a syntax error), so `Credence.Syntax.analyze/2` returns `[]` and `fix_with_trace/2` skips the whole pipeline on the rule's own test inputs — verified end-to-end that `def divide(a, b), do: a div b`, `n = a rem b`, `a div b div c` and `x = a div b + c div d` all parse and come back from `Credence.Syntax.fix_with_trace/2` byte-identical with `applied == []`. Where source genuinely does not parse, `lib/syntax/fix_div_rem.ex` already owns this exact target (same phase, same `@operators ~w(div rem)`, same `analyze`/`infix_use?`/`infix_pattern`/`fix_line`/`rewrite_infix` skeleton, byte-identical message string) and, at equal priority 500, sorts first — so on `expected_sum = n * (n + 1) div 2`, `mid = lo + (hi - lo) div 2`, `x = foo(a, b) div 2` and `do_minto1((n - 1) div 2, 0)` FixDivRem fires and this rule never runs (`applied == ["FixDivRem"]`). The one probed input where it does fire in production is `Enum.map(list, &(&1 div 2))`, which FixDivRem intentionally declines via `infix_in_capture?` ("the regex-based rewrite cannot handle capture context correctly, so we skip the fix") — this rule has no such guard and emits `Enum.map(list, &(&div(1, 2)))`, mangling the capture placeholder `&1` into `&div(1, 2)`; it parses, so nothing downstream catches it. Standalone it is strictly worse than the incumbent because group 1's `\(  [^)]*\)` alternative does not capture the callee, so a function call as left operand glues the name onto the operator: `x = foo(a, b) div 2` → `x = foodiv((a, b), 2)` and `do_minto1((n - 1) div 2, 0)` → `do_minto1div(((n - 1), 2), 0)` (verified both fail to parse), while `mid = lo + (hi - lo) div 2` it leaves unfixed and `IO.puts("a div b")` it rewrites inside the string literal to `IO.puts("div(a, b)")`, silently changing program output. The tests never surface any of this because all six fix tests use bare `word div word` with no call, capture, paren or string operand. The residual safe core — plain `identifier div identifier`, no parens, not in a string — is exactly the subset FixDivRem already rewrites identically (`n = a rem b` → `rem(a, b)` from both), so narrowing to it yields 100% dead code behind the incumbent and there is nothing left to accept.
