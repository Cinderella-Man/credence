defmodule Credence.Semantic.NoRescueInCondCheckTest do
  ...
  test "ignores the :after-in-cond diagnostic (fix only claims rescue/catch)" do
    diag = %{
      severity: :error,
      message: "unexpected option :after in \"cond\"",
      position: {3, 5}
    }
  ...
  test "ignores the :else-in-cond diagnostic (fix only claims rescue/catch)" do
    diag = %{
      severity: :error,
      message: "unexpected option :else in \"cond\"",
      position: {3, 5}
    }
  ...
  test "ignores the case variant, which another rule owns" do
    diag = %{
      severity: :error,
      message: "unexpected option :rescue in \"case\"",
      position: {3, 5}
    }
  ...
  test "ignores warning-severity diagnostics" do
    diag = %{severity: :warning, message: @real_message_rescue, position: {3, 5}}
    refute NoRescueInCond.match?(diag)
  end
