defmodule Credence.Semantic.FixJasonDecodeErrorMessageFieldCheckTest do
  ...
  test "matches the diagnostic" do
    diag = %{severity: :error, message: @real_message, position: {148, 16}, file: "credence_check.ex"}
  ...
  test "attributes the issue to this rule" do
    diag = %{severity: :error, message: @real_message, position: {148, 16}, file: "credence_check.ex"}
    assert FixJasonDecodeErrorMessageField.to_issue(diag).rule == :fix_jason_decode_error_message_field
  end

  test "sets the line in issue meta" do
    diag = %{severity: :error, message: @real_message, position: {42, 16}, file: "credence_check.ex"}
