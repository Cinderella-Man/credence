defmodule Credence.Semantic.NoMessageAccessOnRescueVariableCheckTest do
  ...
  test "ignores an unknown-key .message warning that is not a bare rescue" do
    # Same "unknown key .message" text, but no bare-rescue hint — the
    # structural fix would leave this untouched, so check must not flag it.
    diag = %{
      severity: :warning,
      message:
