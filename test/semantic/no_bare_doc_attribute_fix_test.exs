defmodule Credence.Semantic.NoBareDocAttributeFixTest do
  ...
  test "removes only the diagnostic's bare @doc, not @doc text inside a docstring" do
    input = """
    defmodule Solution do
      @doc \"""
      Example usage:
  ...
  test "leaves source unchanged when the diagnostic line is not a bare @doc" do
    input = """
    defmodule Solution do
      @doc "Calculates total cost."
      def calculate_total_cost(price) do
