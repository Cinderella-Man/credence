defmodule Credence.Semantic.FixHallucinatedMapUpdateArityCheckTest do
  ...
  test "matches Map.update/3 diagnostic without Did you mean" do
    diag = %{
      severity: :warning,
      message: "Map.update/3 is undefined or private",
      position: {1, 1}
    }
  ...
  test "ignores Map.update/4 (correct arity, some other issue)" do
    diag = %{
      severity: :warning,
      message: "Map.update/4 is undefined or private",
      position: {1, 1}
    }
  ...
  test "ignores Map.update/2 (different hallucinated arity)" do
    diag = %{
      severity: :warning,
      message: "Map.update/2 is undefined or private",
      position: {1, 1}
    }
  ...
  test "ignores a user module whose path merely ends in Map" do
    diag = %{
      severity: :warning,
      message: "MyApp.Map.update/3 is undefined or private",
      position: {1, 1}
    }
  ...
  test "should_report? is true when the fix would rewrite the source" do
    ...
    diag = %{severity: :warning, message: @real_message, position: {3, 9}}
    assert FixHallucinatedMapUpdateArity.should_report?(diag, source)
  end

  test "should_report? is false for the piped hallucination the fix skips" do
    source = """
    ...
    refute FixHallucinatedMapUpdateArity.should_report?(diag, source)
  end
end
