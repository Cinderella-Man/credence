defmodule Credence.Semantic.NoHallucinatedSelfBangCheckTest do
  ...
  @real_diag %{severity: :error, message: @real_message, position: {75, 7}, file: "credence_check.ex", stacktrace: [{CancellablePriorityQueue, :handle_call, 3, [file: "credence_check.ex", column: 7, line: 75]}], source: "credence_check.ex", span: {75, 12}}
  ...
  test "ignores generic compile error wrapper" do
    diag = %{
      severity: :error,
      message: "credence_check.ex: cannot compile module CancellablePriorityQueue (errors have been logged)",
      position: 0,
  ...
  test "ignores undefined function for other names" do
    diag = %{
      severity: :error,
      message: "undefined function self!/2 (expected M to define such a function or for it to be imported, but none are available)",
      position: {3, 5}
    }
  ...
  test "ignores higher arities whose message contains self!/1 as a substring" do
    diag = %{
      severity: :error,
      message: "undefined function self!/12 (expected M to define such a function or for it to be imported, but none are available)",
      position: {3, 5}
    }
