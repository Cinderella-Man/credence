defmodule Credence.Semantic.NoEarlyReturnInUnlessCheckTest do
  use ExUnit.Case
  ...
  test "matches the diagnostic" do
    ...
    assert NoEarlyReturnInUnless.match?(diag)
  end

  test "ignores unrelated diagnostics" do
    diag = %{severity: :error, message: "unrelated", position: {1, 1}}
    refute NoEarlyReturnInUnless.match?(diag)
  end
  ...
  test "attributes the issue to this rule" do
    diag = %{severity: :error, message: @real_message, position: {3, 5}}
    assert NoEarlyReturnInUnless.to_issue(diag).rule == :no_early_return_in_unless
  end

  test "sets the line in issue meta" do
    diag = %{severity: :error, message: @real_message, position: {42, 5}}
    assert NoEarlyReturnInUnless.to_issue(diag).meta.line == 42
  end
