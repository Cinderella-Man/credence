defmodule Credence.Syntax.FixDoBlockFusionAnalyzeTest do
  ...
  describe "flags do-block / one-liner fusions" do
    ...
    test "comma one-liner with a stray trailing end" do
      assert [%Issue{} | _] =
               analyze("""
               defmodule Solution do
  ...
  describe "leaves valid code alone" do
    # A `, do:` one-liner whose value is itself a block (`case`/`cond`/`receive`/
    # `fn`) ends in a LEGITIMATE `end`. Stripping it would corrupt working code,
    # so these must not be flagged.
    test "one-liner wrapping an inner do-block is not flagged" do
      assert analyze("""
             defmodule Solution do
    ...
    test "one-liner wrapping an inner fn is not flagged" do
      assert analyze("""
             defmodule Solution do
