defmodule Credence.Syntax.FixMalformedSpecAnalyzeTest do
  ...
  describe "flags specs where :: is inside the argument parens" do
    test "simple types" do
      assert [%Issue{rule: :malformed_spec}] =
               analyze("@spec foo(integer() :: string())")
    end

    test "the actual LLM pattern from the log" do
      assert [%Issue{}] =
               analyze("@spec max_product(list(integer()) :: integer())")
    end

    test "multiple params" do
      assert [%Issue{}] =
               analyze("@spec add(integer(), integer() :: integer())")
    end

    test "function name with ?" do
      assert [%Issue{}] =
               analyze("@spec valid?(string() :: boolean())")
    end

    test "function name with !" do
      assert [%Issue{}] =
               analyze("@spec save!(map() :: {:ok, map()} | {:error, term()})")
    end

    test "nested parameterized types" do
      assert [%Issue{}] =
               analyze("@spec process(list(list(integer())) :: list(integer()))")
    end
