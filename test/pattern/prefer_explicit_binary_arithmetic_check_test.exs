defmodule Credence.Pattern.PreferExplicitBinaryArithmeticCheckTest do
  ...
  describe "leaves good code alone" do
    ...
    # Narrowing: a div/rem step that is itself part of a longer pipe chain reads
    # naturally as a pipe, so it is deliberately left alone.
    test "div step piped onward is not flagged" do
      assert clean?(PreferExplicitBinaryArithmetic, "x |> div(2) |> Integer.to_string()")
    end

    test "rem fed by an upstream pipe is not flagged" do
      assert clean?(PreferExplicitBinaryArithmetic, "a |> b() |> rem(3)")
    end
  end

  describe "fires regardless of surrounding expression" do
    test "single pipe nested in a call argument" do
      assert flagged?(PreferExplicitBinaryArithmetic, "f(x |> div(n))")
    end
  end
end
