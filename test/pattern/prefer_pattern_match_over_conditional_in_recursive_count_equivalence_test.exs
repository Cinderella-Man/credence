defmodule Credence.Pattern.PreferPatternMatchOverConditionalInRecursiveCountEquivalenceTest do
  ...
  test "strict-equality count keeps === semantics after the rewrite" do
    assert_equivalent_module(@strict,
      rule: PreferPatternMatchOverConditionalInRecursiveCount,
      call: {:count, 2},
      inputs: [
        # === trap: 1.0 must NOT count as a match for target 1 (== would)
        {[1.0], 1},
        {[1], 1},
        {[1, 1.0, 1], 1},
        {[2, 3], 1}
