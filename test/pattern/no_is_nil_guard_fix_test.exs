defmodule Credence.Pattern.NoIsNilGuardFixTest do
  ...
  describe "sole guard, param unused" do
    test "def one-liner" do
      assert fix(NoIsNilGuard, "def foo(x) when is_nil(x), do: :bar") == "def foo(nil), do: :bar"
    end

    test "defp one-liner" do
      assert fix(NoIsNilGuard, "defp foo(x) when is_nil(x), do: :bar") ==
               "defp foo(nil), do: :bar"
    end

    test "multi-param, is_nil on first" do
      assert fix(NoIsNilGuard, "def foo(x, y) when is_nil(x), do: y") == "def foo(nil, y), do: y"
    end

    test "multi-param, is_nil on second" do
      assert fix(NoIsNilGuard, "def foo(x, y) when is_nil(y), do: x") == "def foo(x, nil), do: x"
    end

    test "three params, is_nil on middle" do
      assert fix(NoIsNilGuard, "def foo(x, y, z) when is_nil(y), do: {x, z}") ==
  ...
  describe "combined guard with and" do
    test "is_nil first, other guard kept" do
      assert fix(NoIsNilGuard, "def foo(x, y) when is_nil(x) and is_binary(y), do: :ok") ==
               "def foo(nil, y) when is_binary(y), do: :ok"
    end

    test "is_nil second, other guard kept" do
      assert fix(NoIsNilGuard, "def foo(x, y) when is_binary(y) and is_nil(x), do: :ok") ==
               "def foo(nil, y) when is_binary(y), do: :ok"
    end

    test "both params nil, guard dropped entirely" do
      assert fix(NoIsNilGuard, "def foo(x, y) when is_nil(x) and is_nil(y), do: :ok") ==
  ...
  describe "param used in body" do
    test "one-liner uses nil = param binding" do
      assert fix(NoIsNilGuard, "def foo(x) when is_nil(x), do: inspect(x)") ==
               "def foo(nil = x), do: inspect(x)"
    end

    test "combined guard with param used in body" do
      assert fix(NoIsNilGuard, "def foo(x, y) when is_nil(x) and is_binary(y), do: {x, y}") ==
               "def foo(nil = x, y) when is_binary(y), do: {x, y}"
    end

    test "only the used param gets nil = binding" do
      assert fix(NoIsNilGuard, "def foo(x, y) when is_nil(x) and is_nil(y), do: inspect(x)") ==
  ...
  describe "block form" do
    test "param unused" do
      ...
      assert fix(NoIsNilGuard, code) == expected
    end
  ...
  describe "no-ops" do
    test "already pattern matched nil" do
      ...
      assert fix(NoIsNilGuard, code) == code
    end
