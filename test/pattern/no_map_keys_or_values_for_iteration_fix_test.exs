defmodule Credence.Pattern.NoMapKeysOrValuesForIterationFixTest do
  ...
  describe "callback wrapping — nested" do
    ...
    test "Enum.any? with Map.keys" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.any?(Map.keys(m), fn k -> k > 0 end)") ==
    ...
    test "Enum.map with Map.values" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.map(Map.values(m), fn v -> v + 1 end)") ==
               "Enum.map(m, fn {_k, v} -> v + 1 end)"
    end

    test "Enum.map with Map.keys" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.map(Map.keys(m), fn k -> to_string(k) end)") ==
               "Enum.map(m, fn {k, _v} -> to_string(k) end)"
    end

    test "Enum.flat_map" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.flat_map(Map.values(m), fn v -> [v] end)") ==
               "Enum.flat_map(m, fn {_k, v} -> [v] end)"
    end

    test "Enum.count with predicate" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.count(Map.values(m), fn v -> v > 0 end)") ==
    ...
    test "&func/1 capture on Map.values" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.map(Map.values(m), &to_string/1)") ==
  ...
  describe "callback wrapping — pipe" do
    ...
    test "Map.keys |> Enum.map(&func/1)" do
      assert fix(NoMapKeysOrValuesForIteration, "Map.keys(map) |> Enum.map(&to_string/1)") ==
  ...
  describe "count / empty?" do
    test "Enum.count(Map.values(m))" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.count(Map.values(m))") == "Enum.count(m)"
    end

    test "Enum.count(Map.keys(m))" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.count(Map.keys(m))") == "Enum.count(m)"
    end

    test "pipe: Map.values |> Enum.count()" do
      assert fix(NoMapKeysOrValuesForIteration, "Map.values(map) |> Enum.count()") ==
               "Enum.count(map)"
    end

    test "triple pipe: map |> Map.keys() |> Enum.count()" do
      assert fix(NoMapKeysOrValuesForIteration, "map |> Map.keys() |> Enum.count()") ==
               "Enum.count(map)"
    end

    test "Enum.empty?(Map.values(m))" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.empty?(Map.values(m))") == "Enum.empty?(m)"
    end

    test "Enum.empty?(Map.keys(m))" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.empty?(Map.keys(m))") == "Enum.empty?(m)"
    end
  end
  ...
  describe "max / min — no fix (already idiomatic)" do
    test "Enum.max(Map.values(m)) — not flagged, not rewritten" do
      ...
      assert fix(NoMapKeysOrValuesForIteration, code) == code
    end
  ...
  describe "max_by / min_by with callback" do
    test "Enum.max_by with callback" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.max_by(Map.values(m), fn v -> v * 2 end)") ==
               "elem(Enum.max_by(m, fn {_k, v} -> v * 2 end), 1)"
    end

    test "Enum.min_by with callback" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.min_by(Map.values(m), fn v -> abs(v) end)") ==
  ...
  describe "random" do
    test "Enum.random(Map.values(m))" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.random(Map.values(m))") ==
               "elem(Enum.random(m), 1)"
    end

    test "Enum.random(Map.keys(m))" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.random(Map.keys(m))") ==
               "elem(Enum.random(m), 0)"
    end

    test "pipe: Map.keys |> Enum.random()" do
      assert fix(NoMapKeysOrValuesForIteration, "Map.keys(map) |> Enum.random()") ==
  ...
  describe "filter / reject (exercises ex() extractor fix)" do
    test "Enum.filter(Map.values(m), fn ...)" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.filter(Map.values(m), fn v -> v > 0 end)") ==
               "Enum.map(Enum.filter(m, fn {_k, v} -> v > 0 end), fn {_, v} -> v end)"
    end

    test "Enum.filter(Map.keys(m), fn ...)" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.filter(Map.keys(m), fn k -> k > 0 end)") ==
    ...
    test "pipe: Map.values |> Enum.filter" do
      assert fix(NoMapKeysOrValuesForIteration, "Map.values(m) |> Enum.filter(fn v -> v > 0 end)") ==
               "Enum.map(Enum.filter(m, fn {_k, v} -> v > 0 end), fn {_, v} -> v end)"
    end
  ...
  describe "uniq / dedup" do
    test "Enum.uniq(Map.values(m))" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.uniq(Map.values(m))") ==
               "Enum.map(Enum.uniq_by(m, fn {_, v} -> v end), fn {_, v} -> v end)"
    end

    test "Enum.dedup(Map.keys(m))" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.dedup(Map.keys(m))") ==
    ...
    test "pipe: Map.values |> Enum.uniq()" do
      assert fix(NoMapKeysOrValuesForIteration, "Map.values(map) |> Enum.uniq()") ==
  ...
  describe "take / drop / take_while / drop_while" do
    test "Enum.take(Map.values(m), 3)" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.take(Map.values(m), 3)") ==
               "Enum.map(Enum.take(m, 3), fn {_, v} -> v end)"
    end

    test "Enum.drop(Map.keys(m), 2)" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.drop(Map.keys(m), 2)") ==
    ...
    test "pipe: Map.values |> Enum.take(3)" do
      assert fix(NoMapKeysOrValuesForIteration, "Map.values(map) |> Enum.take(3)") ==
  ...
  describe "reverse / sample / shuffle / slice" do
    test "Enum.reverse(Map.values(m))" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.reverse(Map.values(m))") ==
               "Enum.map(Enum.reverse(m), fn {_, v} -> v end)"
    end

    test "Enum.sample(Map.values(m), 3)" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.sample(Map.values(m), 3)") ==
               "Enum.map(Enum.sample(m, 3), fn {_, v} -> v end)"
    end

    test "Enum.shuffle(Map.keys(m))" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.shuffle(Map.keys(m))") ==
               "Enum.map(Enum.shuffle(m), fn {k, _} -> k end)"
    end

    test "Enum.slice with range" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.slice(Map.values(m), 1..3)") ==
  ...
  describe "frequencies" do
    test "Enum.frequencies(Map.values(m))" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.frequencies(Map.values(m))") ==
               "Enum.frequencies_by(m, fn {_, v} -> v end)"
    end

    test "Enum.frequencies(Map.keys(m))" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.frequencies(Map.keys(m))") ==
  ...
  describe "no-ops" do
    ...
    test "variable callback still removes Map.values" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.map(Map.values(m), my_mapper)") ==
               "Enum.map(m, my_mapper)"
    end

    test "complex capture &(&1 + 1) is converted and wrapped" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.map(Map.values(m), &(&1 + 1))") ==
               "Enum.map(m, fn {_k, x} -> x + 1 end)"
    end

    test "complex piped capture &(&1 + 1) is converted and wrapped" do
      assert fix(NoMapKeysOrValuesForIteration, "m |> Map.values() |> Enum.map(&(&1 + 1))") ==
               "Enum.map(m, fn {_k, x} -> x + 1 end)"
    end
  end
  ...
  describe "Map.keys callback wrapping — single-arg lambdas" do
    test "Enum.all? with Map.keys" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.all?(Map.keys(m), fn k -> is_atom(k) end)") ==
               "Enum.all?(m, fn {k, _v} -> is_atom(k) end)"
    end

    test "Enum.each with Map.keys" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.each(Map.keys(m), fn k -> IO.puts(k) end)") ==
               "Enum.each(m, fn {k, _v} -> IO.puts(k) end)"
    end

    test "Enum.flat_map with Map.keys" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.flat_map(Map.keys(m), fn k -> [k, k] end)") ==
               "Enum.flat_map(m, fn {k, _v} -> [k, k] end)"
    end

    test "Enum.count(Map.keys(m), predicate)" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.count(Map.keys(m), fn k -> k > 0 end)") ==
    ...
    test "Enum.max_by with Map.keys" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.max_by(Map.keys(m), fn k -> abs(k) end)") ==
               "elem(Enum.max_by(m, fn {k, _v} -> abs(k) end), 0)"
    end

    test "Enum.min_by with Map.keys" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.min_by(Map.keys(m), fn k -> abs(k) end)") ==
    ...
    test "Enum.take_while with Map.keys" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.take_while(Map.keys(m), fn k -> k > 0 end)") ==
               "Enum.map(Enum.take_while(m, fn {k, _v} -> k > 0 end), fn {k, _} -> k end)"
    end

    test "Enum.drop_while with Map.keys" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.drop_while(Map.keys(m), fn k -> k < 0 end)") ==
  ...
  describe "Map.keys callback wrapping — captures" do
    test "&Mod.func/1 capture on Enum.all? with Map.keys" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.all?(Map.keys(m), &is_atom/1)") ==
               "Enum.all?(m, fn {x, _v} -> is_atom(x) end)"
    end

    test "&Mod.func/1 capture on Enum.map with Map.keys (nested form)" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.map(Map.keys(m), &to_string/1)") ==
               "Enum.map(m, fn {x, _v} -> to_string(x) end)"
    end

    test "complex capture &(&1 * 2) on Map.keys" do
      assert fix(NoMapKeysOrValuesForIteration, "Enum.map(Map.keys(m), &(&1 * 2))") ==
               "Enum.map(m, fn {x, _v} -> x * 2 end)"
    end

    test "complex piped capture &(&1 > 0) on Map.keys" do
      assert fix(NoMapKeysOrValuesForIteration, "m |> Map.keys() |> Enum.filter(&(&1 > 0))") ==
