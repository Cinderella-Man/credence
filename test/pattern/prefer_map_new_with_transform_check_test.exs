defmodule Credence.Pattern.PreferMapNewWithTransformCheckTest do
  ...
  # The 1-arg `Enum.map` form (`Enum.map(fn ...)`) only carries a collection when a
  # preceding pipe step supplies it. With no such step the fix cannot recover the
  # enumerable (and the code is the invalid `Enum.map/1` anyway), so the check must
  # NOT fire — otherwise it would flag a pair the fix leaves untouched.
  test "leaves head-of-pipe 1-arg Enum.map alone (fix can't supply a collection)" do
    assert clean?(PreferMapNewWithTransform, "Enum.map(fn i -> {i, i} end) |> Map.new()")
  end
