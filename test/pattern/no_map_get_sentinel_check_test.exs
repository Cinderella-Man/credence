defmodule Credence.Pattern.NoMapGetSentinelCheckTest do
  ...
  # ═══════════════════════════════════════════════════════════════════
  # POSITIVE — equality comparisons (existing)
  # ═══════════════════════════════════════════════════════════════════

  describe "flags equality comparisons against sentinel" do
    test "basic != -1" do
      assert flagged?("""
             def run(map) do
  ...
  # ═══════════════════════════════════════════════════════════════════
  # POSITIVE — ordering comparisons (NEW)
  # ═══════════════════════════════════════════════════════════════════

  describe "flags ordering comparisons where sentinel is used as domain filter" do
    ...
    test "var >= 0 — non-sentinel constant threshold" do
      assert flagged?("""
             def run(map) do
               val = Map.get(map, :key, -1)
    ...
    test "ordering inside compound condition without equality check" do
      assert flagged?("""
    ...
    test "ordering with -999 sentinel" do
      assert flagged?("""
             def run(map, threshold) do
               val = Map.get(map, :key, -999)
               if val >= threshold, do: val, else: 0
  ...
  describe "does not flag ordering comparison against sentinel literal itself" do
    test "val > -1 — compares against sentinel directly" do
      assert clean?("""
             def run(map) do
               val = Map.get(map, :key, -1)
    ...
    test "val < -1 — compares against sentinel directly" do
      assert clean?("""
             def run(map) do
               val = Map.get(map, :key, -1)
               if val < -1, do: :very_negative, else: val
             end
             """)
    end

    test "-1 < val — flipped, still against sentinel" do
      assert clean?("""
             def run(map) do
               val = Map.get(map, :key, -1)
               if -1 < val, do: val, else: :negative
             end
             """)
    end
  end
  ...
  describe "does not flag scope mismatches" do
    ...
    test "variable rebound between Map.get and ordering comparison" do
      assert clean?("""
             def run(map, threshold) do
               val = Map.get(map, :key, -1)
               val = transform(val)
  ...
  describe "does not flag different variable names" do
    ...
    test "ordering comparison on different variable" do
      assert clean?("""
             def run(map, threshold) do
               val = Map.get(map, :key, -1)
               other = get_other()
               if other >= threshold, do: other, else: 0
             end
             """)
    end
  end
