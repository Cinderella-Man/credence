  ...
  describe "NoRedundantNegatedGuard" do
    ...
    test "detects the find_missing pattern" do
      ...
      defmodule Bad do
        ...
        defp compare_lists([missing_value | _rest1], [val2 | _rest2]) when missing_value != val2,
          do: missing_value
      end
      """
    ...
    test "does not flag different variable names in guards" do
      ...
      # But wait — same position, different names
      # The rule compares by name, so a != c means no match → no flag
      assert check(code) == []
    end
